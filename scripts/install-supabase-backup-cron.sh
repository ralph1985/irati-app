...
SCHEDULE="${IRATI_SUPABASE_BACKUP_CRON_SCHEDULE:-0 0 * * *}"
LOCK_FILE="${IRATI_SUPABASE_BACKUP_LOCK_FILE:-/tmp/irati-supabase-backup.lock}"
CRON_COMMAND="/usr/bin/flock -n $LOCK_FILE $REPO_ROOT/scripts/backup-supabase.sh >> $LOG_DIR/supabase-backup.cron.log 2>&1"
